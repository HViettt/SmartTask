    ...
    <div className="min-h-screen bg-brand-base flex items-center justify-center p-4 relative overflow-hidden font-sans text-xs cyber-grid auth-page">
      ...
      <div className="max-w-sm w-full relative z-10">
        <div className="bg-brand-card border border-brand-border p-8 shadow-2xl relative overflow-hidden hud-border scan-lines animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <Link
            to="/login"
            className="inline-flex items-center gap-1.5 text-[9px] font-bold text-brand-primary-text hover:text-brand-primary uppercase tracking-wider transition-colors mb-6 font-mono border border-brand-border px-2 py-1 bg-brand-base/50"
          >
            <ArrowLeft size={12} className="flex-shrink-0" /> {t('auth.forgot.back')}
          </Link>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div className="flex items-center gap-2 p-3 border border-brand-high bg-brand-high/5 text-brand-high-text text-xs mb-6 font-mono">
                ...
                <span className="uppercase tracking-wide">ERR_FORGOT: {error}</span>
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  disabled={isLoading}
                  className="w-full pl-9 pr-4 py-2 border border-brand-border bg-brand-base text-brand-main focus:border-brand-primary text-xs outline-none transition-colors placeholder-brand-sub/40 font-sans rounded-none disabled:opacity-50"
