  ...
  return (
    <div className="relative font-mono">
      <button
        ...
        className="relative p-2 text-brand-sub hover:bg-brand-base hover:text-brand-main border border-brand-border bg-brand-card transition-colors duration-150 rounded-none"
        aria-label="Notifications"
      >
        <Bell size={16} className={unreadCount > 0 ? "animate-pulse" : ""} />
        {/* Badge count */}
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[14px] h-3.5 px-0.5 text-[8px] font-mono font-bold text-white bg-brand-high border border-brand-card rounded-none shadow-[0_0_8px_rgba(255,23,68,0.3)]">
            {unreadCount}
          </span>
        )}
      </button>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 z-50 top-full mt-2 w-80 sm:w-96 glass-panel border border-brand-border shadow-2xl animate-in fade-in slide-in-from-top-2 duration-200 rounded-xl overflow-hidden glow-primary">
            {/* HUD Corner Tag */}
            <div className="absolute top-2 right-8 text-[6px] text-brand-sub/40 uppercase tracking-widest">[SYS-ALERT-CENTER]</div>
            
            <div className="flex items-center justify-between px-5 py-3 border-b border-brand-border bg-brand-base/30">
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-brand-main">
                {t('notifications.title')}
              </h3>
              <div className="flex items-center gap-2 text-[9px] font-bold">
                <button
                  onClick={() => setIsSettingsOpen(true)}
                  className="p-1 border border-brand-border bg-brand-card text-brand-sub hover:text-brand-main transition-colors"
                ...
                {unreadCount > 0 && (
                  <button
                    className="text-brand-primary-text hover:text-brand-primary transition-colors font-bold uppercase tracking-wider underline underline-offset-4"
